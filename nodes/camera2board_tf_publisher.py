...
class board_publisher():
    ...
    def __init__(self, camera2board_pub,fiducial_markers_pub):

        # Inputs
        self.camera2board_pub = camera2board_pub
        self.fiducial_markers_pub = fiducial_markers_pub
    ...
    def runner(self, data):
        ...
        try:
            ...
            center_blue, center_green, center_red = self.detectColored_Markers(cv_image)
    ...
    def detectColored_Markers(self,image):
     ...
     #------------ Everything needed for matchTemplate() ^^^
        ...
        try:
            msg_img = self.bridge.cv2_to_imgmsg(g_box_image, 'bgr8')
        except CvBridgeError as e:
            print(e)

        self.fiducial_markers_pub.publish(msg_img)
...
def main():
    ...
    # Setup Publishers
    pub_fiducial_markers = rospy.Publisher("Fiducial_Markers", Image, queue_size=20)
    pub_camera2board = rospy.Publisher("/tf", tf2_msgs.msg.TFMessage, queue_size=20)

    # Setup Listeners
    bp_callback = board_publisher(pub_camera2board,pub_fiducial_markers)
